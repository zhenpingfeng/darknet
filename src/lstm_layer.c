...
void backward_lstm_layer_gpu(layer l, network state)
{
    ...
    if (state.delta_gpu.mem) state.delta_gpu.inc(state.delta_gpu, l.inputs*l.batch*(l.steps - 1), l.inputs*l.batch);
    ...
    for (i = l.steps - 1; i >= 0; --i) {
        ...
        if (state.delta_gpu.mem) state.delta_gpu.dec(state.delta_gpu, l.inputs*l.batch, l.inputs*l.batch);
